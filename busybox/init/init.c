/* vi: set sw=4 ts=4 fileencoding=gbk : */
/*
...
static void console_init(void)
{
	...
	if (s) {
		...
		dev = makedev(MAJ_CONSOLE, MIN_CONSOLE);
		fd = open(s, O_RDWR | O_NONBLOCK | O_NOCTTY);
		if (fd < 0) {
			mknod(DEV_CONSOLE, (S_IFCHR | S_IRUSR | S_IWUSR), dev);
			fd = open(DEV_CONSOLE, O_RDWR | O_NONBLOCK | O_NOCTTY);
		}
		if (fd >= 0) {
